...
The factory construction should not be treated as accepted merely because one
representative factory path succeeds. A candidate deployment must pass an
end-to-end devnet acceptance matrix in which every value-bearing factory
business flow is exercised as a committed transition, and every attack-shaped
factory rejection is checked against its intended error class. This section
states that matrix at protocol level. It deliberately does not prescribe a
particular transaction builder, storage format, or proof encoding.

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{0.25\linewidth}L{0.39\linewidth}L{0.25\linewidth}}
\toprule
...
\bottomrule
\end{tabular}
\caption{Factory acceptance matrix. Every row is a required devnet acceptance family for a candidate deployment of the factory protocol.}
\label{tab:factory-acceptance}
\end{table}

The acceptance rule is strict: every positive row in
Table~\ref{tab:factory-acceptance} must be represented by committed on-chain
evidence, and every negative row must be represented by an exact rejection
class. A paper proof of non-interference is therefore not enough on its own. It
must be accompanied by acceptance evidence showing that conservative paths,
...
\begin{enumerate}[label=E\arabic*.]
  ...
  \item factory acceptance evidence for every lifecycle, reduced-rights, reduced-exit, conservative-splice, reduced-splice, asymmetric, and one-sided typed path in Table~\ref{tab:factory-acceptance};
